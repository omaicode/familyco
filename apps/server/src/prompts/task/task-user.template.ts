...
export function renderTaskUserPrompt(input: TaskUserPromptInput): string {
  ...
  lines.push(
    ...
    '',
    '**Step 0 — Scan workspace first (mandatory):** Before any write action, inspect the current workspace tree and key files to understand existing structure and conventions.',
    '  - Start by listing top-level folders/files in the project workspace.',
    '  - Identify relevant existing files (entry HTML, webpack config, package scripts, assets, output folder).',
    '  - Do NOT create new files until this scan is complete.',
    '',
    alreadyInProgress
    ...
    '',
    '**Step 2 — Plan ordered implementation:** Create a concise file-by-file action sequence so changes are applied in a deterministic order.',
    '',
    ...
    '',
    '**Step 6 — Final reply:** Write a Markdown summary of: what was accomplished, what decisions were made, and the final status. Use bullet points and headers for readability.',
    ...
    '',
    '> Steps 0, 1, 4, 5, and 6 are REQUIRED. Do not stop before completing all of them.'
  );
